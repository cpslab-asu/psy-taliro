...
class NextTestCase(ParserTestCase):
    ...
    def test_next(self) -> None:
        self.assertIsNext(parse(r"next pred1", self._preds))

    def test_next_alternate_synax(self) -> None:
        self.assertIsNext(parse(r"next X pred1", self._preds))

    def test_next_with_and(self) -> None:
        self.assertIsNext(parse(r"next (pred1 && pred2)", self._preds))

    def test_nested_next_with_and(self) -> None:
        self.assertIsNext(parse(r"X (pred1 && X X X pred3)", self._preds))

    def test_many_next(self) -> None:
        self.assertIsNext(parse(r"X X X X X X X X (pred1 & pred2)", self._preds))
