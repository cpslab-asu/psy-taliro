...
@skipIf(_can_parse is False, "Global parsing test case is not available without parsing functionality")
class GloballyTestCase(ParserTestCase):
    def assertIsGlobal(self, value):
        import tltk_mtl as mtl

        self.assertIsInstance(value, mtl.Global)

    def test_globally_infinite(self) -> None:
        self.assertIsGlobal(parse(r"[](1, inf) pred1", self._preds))
    ...
    def test_globally_with_and(self) -> None:
        self.assertIsGlobal(parse(r"G(1, 1000) (pred1 && pred2)", self._preds))

    def test_globally_with_or(self) -> None:
        self.assertIsGlobal(parse(r"[](25, 3000) (pred2 | pred3)", self._preds))
